...
function checkAll() {
	$(".checkAll").change(function () {
		if ($(this).prop("checked")) {
			$(".storCheck").prop("checked", true)
			$(".goodCheck").prop("checked", true)
			$(".checkAll").prop("checked", true)
		} else {
			$(".storCheck").prop("checked", false)
...
function getSum() {
	...
	var money = 0;
	//总数量
	$(".checkAll:checked").parents("#spAce").find("#numBer").each(function (i, ele) {
		count += parseInt($(ele).val());
	})
	$(".Num1").text(count);
	//总计
	$(".checkAll:checked").parents(".shoppingBox").find("#money").each(function (i, ele) {
		money += parseFloat($(ele).text().substr(1));
	})
	$("#spAce").text("￥" + money.toFixed(2));
